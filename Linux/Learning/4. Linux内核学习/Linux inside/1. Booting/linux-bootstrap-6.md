...
这个结构体提供了关于内存映射的信息. 你可能还记得, 在前面的部分, 我们已经建立了初始的从 0 到`4G`的页表. 现在我们可能需要访问`4G`以上的内存来在随机的位置加载内核. 所以, `initialize_identity_maps`函数初始化一个内存区域, 它用于可能需要的新页表. 首先, 让我们尝试查看`x86_mapping_info`结构体的定义.
