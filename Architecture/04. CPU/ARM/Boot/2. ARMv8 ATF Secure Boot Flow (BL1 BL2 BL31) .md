...
* **[BL2 - Boot Firmware]**: 可视为 ARMv7 SPL 的流程, 此时 **DDR 还没有初始化**, 这一步总需要被加载到**片内的 SRAM 执行**, 一般在这个阶段会**完成 DDR 的初始化**, 因此**后面的 image 可以被加载到 DDR** 中. BL3 所有的 image 都是由 BL2 加载的, BL31/BL32 是可选的配置. 若没有 trusted os 可以没有 BL2, 若不支持 EL3 异常等级及 secure monitor, 则可以去掉 BL31.
