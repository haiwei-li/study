...
- [1. boot high-level](#1-boot-high-level)
	- [1.1. boot overview](#11-boot-overview)
	- [1.2. exception switching](#12-exception-switching)
	- [1.3. memory layout](#13-memory-layout)
- [2. SoC BL1 Booting](#2-soc-bl1-booting)
	- [2.1. el3\_entrypoint\_common flow](#21-el3_entrypoint_common-flow)
		- [2.1.1. sctlr\_el3 config](#211-sctlr_el3-config)
		- [2.1.2. cold\_boot and warm\_boot](#212-cold_boot-and-warm_boot)
		- [2.1.3. PIE dealing](#213-pie-dealing)
		- [2.1.4. exception config](#214-exception-config)
			- [2.1.4.1. exception vector](#2141-exception-vector)
			- [2.1.4.2. reset handler](#2142-reset-handler)
		- [2.1.5. platform level init](#215-platform-level-init)
		- [2.1.6. secondary cpu](#216-secondary-cpu)
		- [2.1.7. memory init](#217-memory-init)
		- [2.1.8. C env init](#218-c-env-init)
		- [2.1.9. 运行栈设置](#219-运行栈设置)
	- [2.2. bl1\_setup](#22-bl1_setup)
		- [2.2.1. bl1\_early\_platform\_setup](#221-bl1_early_platform_setup)
		- [2.2.2. bl1\_plat\_arch\_setup](#222-bl1_plat_arch_setup)
	- [2.3. bl1\_main](#23-bl1_main)
		- [2.3.1. bl1\_arch\_setup](#231-bl1_arch_setup)
		- [2.3.2. secure boot init](#232-secure-boot-init)
		- [2.3.3. bl1\_platform\_setup](#233-bl1_platform_setup)
		- [2.3.4. load image](#234-load-image)
			- [2.3.4.1. get image id](#2341-get-image-id)
			- [2.3.4.2. bl2 image load](#2342-bl2-image-load)
			- [2.3.4.3. next stage image preparing](#2343-next-stage-image-preparing)
	- [2.4. el3\_exit](#24-el3_exit)
- [3. SoC BL2 Booting](#3-soc-bl2-booting)
	- [3.1. bl2 init](#31-bl2-init)
		- [3.1.1. parameters](#311-parameters)
		- [3.1.2. exception setting](#312-exception-setting)
		- [3.1.3. set sctlr\_el1 register](#313-set-sctlr_el1-register)
		- [3.1.4. C env and stack](#314-c-env-and-stack)
	- [3.2. bl2 platform setup](#32-bl2-platform-setup)
	- [3.3. bl2 image load](#33-bl2-image-load)
		- [3.3.1. before  load](#331-before--load)
		- [3.3.2. behind load](#332-behind-load)
		- [3.3.3. params settings](#333-params-settings)
	- [3.4. bl1 handle next-image](#34-bl1-handle-next-image)
- [4. SoC BL31 Booting](#4-soc-bl31-booting)
	- [4.1. bl31 init](#41-bl31-init)
		- [4.1.1. params saving](#411-params-saving)
	- [4.2. bl31 common](#42-bl31-common)
	- [4.3. bl31 main](#43-bl31-main)
		- [4.3.1. bl31\_early\_platform\_setup2](#431-bl31_early_platform_setup2)
		- [4.3.2. bl31\_plat\_arch\_setup](#432-bl31_plat_arch_setup)
		- [4.3.3. bl31\_platform\_setup](#433-bl31_platform_setup)
		- [4.3.4. ehf init](#434-ehf-init)
		- [4.3.5. 运行时服务初始化](#435-运行时服务初始化)
	- [4.4. bl32 start](#44-bl32-start)
	- [4.5. bl33 booting](#45-bl33-booting)
- [5. Ref](#5-ref)
...
* `[BL2 - Boot Firmware]`: 可视为 ARMv7 SPL 的流程, 此时 **DDR 还没有初始化**, 这一步总需要被加载到**片内的 SRAM 执行**, 一般在这个阶段会**完成 DDR 的初始化**, 因此**后面的 image 可以被加载到 DDR** 中. BL3x 所有的 image 都是**由 BL2 加载**的, `BL31/BL32` 是可选的配置. 若**没有 trusted os** 可以**没有 BL2**, 若不支持 EL3 异常等级及 secure monitor, 则可以去掉 **BL31**.
