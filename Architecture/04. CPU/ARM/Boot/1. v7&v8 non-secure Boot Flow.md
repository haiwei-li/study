...
* **TPL** 是 **uboot** 引申出来的一级引导, 由于 SPL 是要被放到**SoC**上面的**SRAM**执行的, 但是**由于 SRAM 空间有限**, 故将 SPL 内部的功能进行拆分完成, 这个在 uboot 文档中介绍主要给 **PowerPC** 这种**空间不足**的架构平台使用的, 而 ARM 平台**很少定义 TPL**[^10].
...
cpu_init_cp15,cpu_init_cp15 主要用于对 cp15 协处理器进行初始化, 其主要目的就是关闭其 MMU 和 TLB, 如下: https://github.com/ARM-software/u-boot/blob/master/arch/arm/cpu/armv7/start.S#L139
...
(3)跳转到_main 处执行, 该函数的定义位于 arch/arm/lib/crt0_64.S 中. 它主要是初始化 c 语言的执行环境, crt 的意思即为 c run time.
