...
## 6. 配置 LPI
仅在启用亲和路由时才支持 LPI, 并且其配置方式与其他中断类型不同.

配置 LPI 需要设置:
 - 再分发器
 - 可选的 ITS(中断转换服务)

LPI 始终是基于消息的中断, 可以由 ITS 支持. ITS 负责从外设接收中断, 并将它们作为 LPI 转发到适当的再分发器. 一个系统可能包含多个 ITS, 在这种情况下, 每个 ITS 都必须单独配置. 外设也可以绕过 ITS, 直接将 LPI 发送到再分发器. 然而, ITS 提供了许多功能, 便于高效处理大量中断源.
